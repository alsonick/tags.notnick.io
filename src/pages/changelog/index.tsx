    ...
    <Container>
      ...
      <MainWrapper>
        ...
        <div className="mt-12 flex flex-col gap-16 mb-auto">
          {entries.length === 0 && (
            <p className="text-lg text-gray-800">No changelog entries yet — check back soon!</p>
          )}
          {entries.map((entry) => (
            <section key={entry.slug}>
              {entry.commit && (
                <Link
                  className="inline-block bg-gray-100 hover:bg-gray-200 text-gray-800 font-mono text-sm px-2.5 py-0.5 rounded-md transition-colors mb-2"
              ...
              <h2 className="text-3xl tracking-tight">
                ...
                {entry.date && <span className="text-gray-500"> ({formatChangelogDate(entry.date)})</span>}
              </h2>
              <div className="border-l-2 border-blue-600 pl-8 ml-1 mt-6 text-gray-800">
